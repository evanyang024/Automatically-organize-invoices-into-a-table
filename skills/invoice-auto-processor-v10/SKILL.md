...
| 30 | **铁路电子客票未检测导致项目名为空** — 发票文本含"电子发票（铁路电子客票）"标识但 parser 未检测，id掩码过滤后 items 为空，重命名文件缺项目名。v10 修复：`parse_invoice()` 入口处检测文本是否含「铁路电子客票」，是则直接调 `_parse_train_ticket()`，跳过标准解析流程。铁路客票项目名固定为"铁路客运"。|
...
| `references/parser.py` | 发票字段解析 (v10: ¥三级回退 + 竖排 + 星号跨行 + 铁路客票专用解析 + 非税收入 + 通行费 + 身份证号掩码过滤) |
